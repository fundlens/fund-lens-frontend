...
export default function DistrictCandidatesPage() {
  const { state, district: districtParam } = useParams<{ state: string; district: string }>()
  const location = useLocation()
  const stateCode = state?.toUpperCase() || 'MD'
  const stateName = STATE_NAMES[stateCode] || stateCode

  // Extract district number from the parameter (e.g., "md-01" -> "01")
  const district = districtParam?.split('-')[1] || districtParam
  ...
  usePageTitle(`U.S. House - ${stateCode}-${district}`)

  const breadcrumbs = [
    { label: 'Home', path: '/' },
    ...(isNewStructure ? [{ label: 'Federal Races', path: '/federal' }] : []),
    { label: stateName, path: baseStatePath },
    { label: 'U.S. House', path: `${baseStatePath}/us-house` },
    { label: `${stateCode}-${district}` }
  ]

  const [displayLimit, setDisplayLimit] = useState(10)

  // Fetch district candidates with stats
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading district candidates...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {displayedCandidates.length > 0 && (
        <div>
          <h2 className="text-xl font-semibold text-gray-900 mb-4">
            Top Fundraisers - U.S. House - {stateCode}-{district}
          </h2>
      ...
      {displayedCandidates.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-600">No candidates found for {stateCode}-{district}.</p>
        </div>
      )}
    </div>
  )
