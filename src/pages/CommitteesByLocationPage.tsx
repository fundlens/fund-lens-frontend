...
export default function CommitteesByLocationPage() {
  ...
  usePageTitle(`${stateName} Committees`)

  // Fetch committees filtered by state, sorted by total received
  ...
  // Get unique committee types that have received contributions
  const availableTypes = Array.from(
    new Set(
      committees
        ?.filter(c => c.stats && c.stats.total_amount_received > 0)
        .map(c => c.committee_type)
    )
  ).sort()

  // Filter to only committees with fundraising > $0, and optionally matching type
  const filteredCommittees = committees?.filter(c =>
    c.stats &&
    c.stats.total_amount_received > 0 &&
    (!typeCode || c.committee_type === typeCode)
  ) || []

  const addTypeFilter = (committeeType: string) => {
    const newParams = new URLSearchParams(searchParams)
    newParams.set('type', committeeType.toLowerCase())
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="mb-6">
        <h1 className="text-4xl font-bold text-gray-900 mb-4">
          {stateName} Committees
        </h1>
        <p className="text-lg text-gray-600">
          Top fundraising committees in {stateName}
        </p>
      </div>
      ...
      <div className="mb-8">
        <div className="bg-white rounded-lg shadow border border-gray-200 p-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {!typeCode && availableTypes.length > 0 && (
                  ...
                  <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg border border-gray-200 max-h-96 overflow-y-auto z-10">
                    <div className="p-2">
                      {availableTypes.map((type) => (
                        ...
                        >
                          {type} - {getCommitteeTypeLabel(type)}
                        </button>
                      ))}
                    </div>
                  </div>
      ...
      <section>
        {isLoading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-600">No committees found in {stateName}.</p>
          </div>
        )}
      </section>
    </div>
